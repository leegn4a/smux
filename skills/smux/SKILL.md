...
Before `type`, `message`, or `keys`, run `read <target>`. Each action consumes
that read permission, so read again before the next action. `type` and `message`
only enter text; use `keys <target> Enter` to submit it.
