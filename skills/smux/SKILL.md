...
consumes that read permission, so read again before the next action. `exec` and
`message` submit immediately; use `type`, then `read` and `keys Enter` when the
entered text needs verification before submission.
...
`message` adds the sender pane and reply target, then presses Enter. When
receiving one, reply to the `pane:%N` target in its header using the same
sequence.
...
| `message <target> <text>` | Send and submit text with sender and reply details |
